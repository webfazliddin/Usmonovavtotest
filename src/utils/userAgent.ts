...
export const CHROMIUM_VERSION = (() => {
  try {
    return navigator.userAgent && navigator.userAgent.match(/Chrom(?:e|ium)\/(.+?)(?:\s|\.)/)?.[1];
  } catch (err) {}
})();

export const IS_APPLE_MOBILE =
  (/iPad|iPhone|iPod/.test(navigator.platform) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1)) && !(ctx as any).MSStream;

export const IS_SAFARI =
  !!("safari" in ctx) ||
  !!(USER_AGENT && (/\b(iPad|iPhone|iPod)\b/.test(USER_AGENT) || (!!USER_AGENT.match("Safari") && !USER_AGENT.match("Chrome")))); /*  || true */
