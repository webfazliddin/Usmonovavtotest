...
import { IExamAttemptData } from "./types";
...
const correctAnswersCount = computed(() =>
  attempt.value.filter(q => q.isCorrect).length
);

const percentage = computed(() =>
  Math.round((correctAnswersCount.value / attempt.value.length) * 100)
);
...
const fetchAttemptResult = () => {
  ExamService.GetAttemptResult(attemptId.value)
    ...
    .catch((e) => {
      notify({
        text: e.response?.data?.message || "Natijani yuklashda xatolik yuz berdi",
        type: "error",
      });
    });
};
...
<template>
  <div class="exam-result-container">
    ...
    <div class="test-footer">
      <button class="footer-btn back-btn" @click="emits('goBack')">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
