...
const fetchAttemp = () => {
  ...
  AttemptService.StartQuestion(url).then((res: AxiosResponse<{ questions: ICategoryAttempData[] }>) => {
    ...
    // Aggressively preload all images on test start
    preloadAllImages();
  });
};

// Aggressively preload ALL images when test starts with batching
const preloadAllImages = () => {
  // First batch: Load first 3 images immediately with high priority
  ...
  // Second batch: Load next 7 images after a short delay
  setTimeout(() => {
    const secondBatch = attempt.value.slice(3, 10);
    secondBatch.forEach((question, index) => {
      if (question?.question?.fileId) {
        const imageUrl = `https://api.uatest.uz/api/Files?fileName=${question.question.fileId}`;
        if (imageCache.has(imageUrl)) return;
  ...
  // Third batch: Load remaining images with low priority
  setTimeout(() => {
...
const checkImageCache = () => {
  ...
  if (imageCache.has(imageUrl)) {
    // Image is already cached, hide loading immediately
    imageLoading.value = false;
  }
};
...
const nextAttemp = () => {
  if (activeQuestion.value?.choiceId) {
    ...
    // Check if next image is already cached
    checkImageCache();
    // Reset description visibility for next question
...
const handleKeyPress = (event: KeyboardEvent) => {
  ...
  // Arrow Right or ArrowRight - go to next question if answer is selected
  ...
  // Arrow Left or ArrowLeft - go to previous question
  ...
  // F1-F12 keys using event.code
  if (event.code.startsWith('F') && event.code.length >= 2) {
    const keyMatch = event.code.match(/^F(\d+)$/);
    if (keyMatch) {
      event.preventDefault();
      const keyNumber = parseInt(keyMatch[1]); // Extract number from F1, F2, etc.
  ...
  <div class="modern-test-container">
    ...
    <div class="test-header">
      <div class="question-progress">
        ...
        <span class="total-questions">/ {{ category.questionsCount }}</span>
      </div>
      <div class="progress-indicator">
        <div class="progress-bar" :style="{ width: `${((activeQuestionIndex + 1) / category.questionsCount) * 100}%` }"></div>
      </div>
      <div class="time-info">
    ...
    <div class="test-footer">
      <button class="footer-btn back-btn" @click="emits('update:modelValue', false)">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="question-navigator" v-if="attempt.length">
        <div
          v-for="(n, i) in category.questionsCount"
          :key="i"
          class="nav-number"
          :class="{
