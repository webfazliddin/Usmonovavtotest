...
const saveData = async (submit: SubmitEventPromise) => {
  ...
  if (valid) {
    ...
    const api = +String(id) ? "PutMarkCategory" : "PostMarkCategory";
    const text = +String(id) ? "Edited" : "Created";
    const payload = +String(id) ? formModel.value : formData
    // @ts-ignore
    MarkCategoryService[api](payload, id)
      .then(() => {
        notify({
          text: t(`successUser${text}`),
          type: "success",
        });
        router.push({ name: "MarkCategory" });
      })
      .catch((e) => {
        setError(e);
      })
...
const fetchUser = () => {
  if (+String(id)) {
    loading.value = true;
    MarkCategoryService.GetById(id)
      .then((res) => {
      ...
      <UiParentCard>
        <v-row>
          <v-col lg="3" md="6" cols="12">
            <FormInput v-model:model-value="formModel.name" :label="$t('markLabel')" required />
          </v-col>
        </v-row>
      </UiParentCard>
      <v-row class="mt-4">
